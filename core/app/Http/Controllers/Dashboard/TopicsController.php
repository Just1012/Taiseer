            ...
            foreach ($WebmasterSection->customFields as $customField) {
                if ($customField->in_statics && ($customField->type == 6 || $customField->type == 7)) {
                    ...
                    foreach ($cf_details_lines as $cf_details_line) {
                        ...
                        $Topics_count = Topic::with('company')
                            ->where('webmaster_id', '=', $WebmasterSection->id)
                            ->wherein('id', $tids)
                            ->count();
    ...
    public function clone($webmasterId, $id)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($Topic)) {
                ...
                if (@Auth::user()->permissionsGroup->edit_status) {
                    return redirect()->action('Dashboard\TopicsController@edit', [$webmasterId, $NewTopic->id])->with(
                        'doneMessage',
                        __('backend.addDone')
                    );
                } else {
