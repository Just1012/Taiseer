    ...
    <div class="padding">
        <div class="box">
            <div class="box-header dker m-b-xs">
                <h3>{{ __('backend.typeActivity') }}</h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    {{ __('backend.webmasterTools') }} /
                    <a href="">{{ __('backend.typeActivity') }}</a>
                </small>
            </div>
            <div class="box-tool">
                <ul class="nav">
                    <li class="nav-item inline">
                        <a class="btn btn-fw primary" href="{{ route('typeActivity.Create') }}">
                            <i class="material-icons">&#xe02e;</i>
                            &nbsp; {{ __('backend.typeActivity') }}</a>
                    </li>
                </ul>
            </div>
            @if ($typeActivity->total() == 0)
                <div class="row p-a">
                    <div class="col-sm-12">
                    ...
                    <table class="table table-bordered m-a-0">
                        <thead class="dker">
                            <tr>
                                ...
                                <th>{{ __('backend.typeActivity') }}</th>
                                <th class="text-center">{{ __('backend.imageFront') }}</th>
                                <th class="text-center">{{ __('backend.imageback') }}</th>
                                <th class="text-center">{{ __('backend.info') }}</th>
                        ...
                        <tbody>
                            ...
                            ?>
                            @foreach ($typeActivity as $WebSection)
                                <?php
                                if ($WebSection->$title_var != '') {
                                    $title = $WebSection->$title_var;
                                    $info = $WebSection->$info_var;
                                } else {
                                    $title = $WebSection->$title_var2;
                                    $info = $WebSection->$info_var2;
                                }
                                ?>
                                <tr>
                                    ...
                                    <td class="h6">
                                        <img style="height: 40px"
                                            src="{{ asset('uploads/topics/' . $WebSection->image_front) }}" alt="">
                                    </td>
                                    <td class="h6">
                                        <img style="height: 40px"
                                            src="{{ asset('uploads/topics/' . $WebSection->image_back) }}" alt="">
                                    </td>
                                    <td class="h6"> {!! $info !!}</td>

                                    <td class="text-center">
                                        <a class="btn btn-sm info"
                                            href="{{ route('typeActivity.Edit', ['id' => $WebSection->id]) }}">
                                    ...
                                    <div class="modal-dialog" id="animate">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <a href="{{ route('typeActivity.updateStatus', ['id' => $WebSection->id]) }}"
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-3 text-center">
                            <small class="text-muted inline m-t-sm m-b-sm">{{ __('backend.showing') }}
                                {{ $typeActivity->firstItem() }}
                                -{{ $typeActivity->lastItem() }} {{ __('backend.of') }}
                                <strong>{{ $typeActivity->total() }}</strong> {{ __('backend.records') }}
                            </small>
                        </div>
