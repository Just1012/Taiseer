            ...
            <div class="box-tool">
                <ul class="nav">
                    <li class="nav-item inline">
                        <a class="btn btn-fw primary" href="{{ route('currency.Create') }}">
                            <i class="material-icons">&#xe02e;</i>
                            &nbsp; {{ __('backend.newCurrency') }}</a>
                    </li>
                </ul>
            </div>
            @if ($currency->total() == 0)
                    ...
                    <table class="table table-bordered m-a-0">
                        <thead class="dker">
                            <tr>
                                ...
                                <th>{{ __('backend.currencyName') }}</th>
                                <th class="text-center">{{ __('backend.currencySymbol') }}</th>
                        ...
                        <tbody>
                            ...
                            ?>
                            @foreach ($currency as $WebSection)
                                <?php
                                ...
                                <tr>
                                    ...
                                    <td class="h6">{{ $WebSection->name }}</td>
                                    <td class="h6">{{ $WebSection->symbol }}</td>
                                    <td class="text-center">
                                        <a class="btn btn-sm info"
                                            href="{{ route('currency.Edit', ['id' => $WebSection->id]) }}">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body text-center p-lg">
                                                <p>
                                                    ...
                                                    <br>
                                                    <strong>[ {{ $WebSection->name }} ]</strong>
                                                </p>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn dark-white p-x-md"
                                                    data-dismiss="modal">{{ __('backend.no') }}</button>
                                                <a href="{{ route('currency.updateStatus', ['id' => $WebSection->id]) }}"
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-3 text-center">
                            <small class="text-muted inline m-t-sm m-b-sm">{{ __('backend.showing') }}
                                {{ $currency->firstItem() }}
                                -{{ $currency->lastItem() }} {{ __('backend.of') }}
                                <strong>{{ $currency->total() }}</strong> {{ __('backend.records') }}
                            </small>
                        </div>
